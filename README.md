...
Через if рассматривается если x > 0, то переменной module(тип int) присваеваем x  без изменений, 
...
<br>
Реализация:<br>
Метод is35(int x)Проверяет, делится ли число на 3 или 5 (но не на оба одновременно).
x (тип int) — проверяемое число.
Возвращаемое значение:
true — если условие выполняется, иначе false (тип boolean).
Использует операторы % и логические условия.
