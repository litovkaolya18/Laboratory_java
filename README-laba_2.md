...
В нём находятся поля для хранения данных: lastName (тип String) - фамилия, firstName (тип String) - имя, patronymic (тип String) - отчество.
...
Свойства (геттеры): которые возращают параметры. В методе toString() происходит основная логика программы. 
...
В классе Employees реализовано два конструктора: первый принимает готовые данные (имя и отдел), второй позволяет ввести имя сотрудника с клавиатуры с проверкой на корректность ввода через класс Check.
в котором работает сотрудник. Класс содержит свойства getName для получения имени сотрудника и getDepartmentEmployees для получения списка всех сотрудников отдела. Метод toString формирует текстовое представление сотрудника: если сотрудник является начальником, выводится "Имя начальник отдела Название", иначе "Имя работает в отделе Название, начальник которого Имя". <br>
Класс Department содержит поля: название отдела, начальник, массив сотрудников и счетчик сотрудников. Конструктор принимает название отдела и инициализирует массив для 100 сотрудников. Свойства включают getName для получения названия, getDirector для получения начальника, setDirector для назначения начальника. <br>
В главном классе Main мы выводим сначало список того, о чём нас требуют в задании, а далее пользователю предоставляется ввести данные самостоятельно.
...
В класс Department были добавленны новые поля: employees - массив для хранения ссылок на всех сотрудников отдела, count - счетчик текущего количества сотрудников в отделе.
...
Но так как в классе Time уже есть всё необходимые свойства для этого задания - getHourse(), getMinutes() и getsecondsFromMinutes(), то класс остаётся не изменным.
